        ...
        <div className="container flex h-14 items-center">
          <div className="flex items-center gap-2">
            <Link to="/" className="flex items-center gap-2">
              <img alt="ChroMarx Logo" className="h-7 w-auto" src="/lovable-uploads/69941785-8105-45c5-950c-818f41e9c5d3.png" />
            ...
            <nav className="flex items-center gap-2">
              ...
              {isMobile ? <div className="flex items-center gap-3">
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="ghost" size="sm" className="h-9 w-9 rounded-full p-0 relative touch-target" aria-label="Notifications">
                        ...
                        {showNotificationDot && <span className="absolute top-0 right-0 h-2.5 w-2.5 bg-red-500 rounded-full ring-2 ring-background" />}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-[90vw] max-w-[350px] p-0 border border-border/60 shadow-lg rounded-2xl bg-background/95 backdrop-blur-md notification-popover" align="end" alignOffset={-8} sideOffset={16}>
                      <div className="flex items-center justify-between px-4 py-3 border-b border-border/40 bg-primary/5 rounded-t-2xl">
                        <h3 className="font-medium text-sm flex items-center gap-1.5">
                          ...
                          Notifications
                          {unreadCount > 0 && <Badge variant="secondary" className="ml-1 text-xs py-0 h-5 bg-primary/10 text-primary">
                              {unreadCount} new
                            </Badge>}
                        </h3>
                        {unreadCount > 0 && <Button variant="ghost" size="sm" className="h-7 text-xs font-normal touch-target hover:bg-primary/10" onClick={markAllAsRead}>
                      ...
                      <ScrollArea className="h-[min(70vh,350px)]">
                        {notifications.length > 0 ? <div className="py-1 w-full">
                            {notifications.map(notification => <div key={notification.id} className="notification-item px-4 py-3 hover:bg-muted/50 active:bg-muted transition-colors touch-target w-full box-border" onClick={() => markAsRead(notification.id)}>
                                  ...
                                  <div className="space-y-1 flex-1 min-w-0 pr-2">
                                    <div className="flex items-center justify-between gap-1.5">
                                      <p className="text-sm font-semibold truncate pr-1 flex items-center gap-1.5">
                                        {notification.title}
                                        {!notification.read && <span className="w-1.5 h-1.5 bg-primary rounded-full flex-shrink-0" />}
                                      </p>
                          ...
                          </div> : <div className="flex flex-col items-center justify-center h-full py-8">
                      ...
                      <div className="px-4 py-3 border-t border-border/40 bg-muted/30 rounded-b-2xl">
                        <Link to="/notifications" className="flex items-center justify-center text-xs font-medium text-primary hover:underline gap-1 touch-target h-9" onClick={() => document.body.click()} // Close the popover
                    >
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-[350px] p-0 border border-border/60 shadow-lg rounded-2xl bg-background/95 backdrop-blur-md" align="end" alignOffset={-5} sideOffset={16}>
                      <div className="flex items-center justify-between px-4 py-3 border-b border-border/40 bg-primary/5 rounded-t-2xl">
                        <h3 className="font-medium text-sm flex items-center gap-1.5">
                          <Bell className="h-3.5 w-3.5 text-primary" />
                          Notifications
                        ...
                        {unreadCount > 0 && <Button variant="ghost" size="sm" className="h-7 text-xs font-normal hover:bg-primary/10" onClick={markAllAsRead}>
                            <Check className="h-3.5 w-3.5 mr-1" />
                            Mark all read
                          </Button>}
                      </div>
                      ...
                      <ScrollArea className="h-[300px]">
                        {notifications.length > 0 ? <div className="py-1">
                            {notifications.map(notification => <div key={notification.id} className={`px-4 py-3 hover:bg-muted/50 cursor-pointer transition-colors ${!notification.read ? 'bg-primary/5' : ''}`} onClick={() => markAsRead(notification.id)}>
                      ...
                      <div className="px-4 py-3 border-t border-border/40 bg-muted/30 rounded-b-2xl">
                        <Link to="/notifications" className="flex items-center justify-center text-xs font-medium text-primary hover:underline gap-1 h-8" onClick={() => document.body.click()} // Close the popover
                    >
                          View all notifications
                        </Link>
                      </div>
